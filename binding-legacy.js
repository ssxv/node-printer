...
function tryNodeGypRebuild() {
  try {
    ...
    const res = child.spawnSync(
      process.execPath,
      [require.resolve('node-gyp/bin/node-gyp'), 'rebuild', '--target=node_printer_legacy'],
      {
        stdio: 'inherit',
        cwd: __dirname
      }
...
function loadBinding() {
  ...
  // 4. cannot load
  throw new Error(
    'Failed to load legacy node_printer binding. Tried build/Release/node_printer_legacy.node, prebuild-install, and node-gyp rebuild. Ensure native build succeeded or install prebuilt binaries.'
  );
}

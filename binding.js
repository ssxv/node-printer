...
function tryNodeGypRebuild() {
	try {
		...
		const res = child.spawnSync(process.execPath, [require.resolve('node-gyp/bin/node-gyp'), 'rebuild'], {
			stdio: 'inherit',
			cwd: __dirname
		});
		if (res.status === 0) {
...
function loadBinding() {
	...
	// 4. cannot load
	throw new Error('Failed to load native node_printer binding. Tried build/Release, prebuild-install, and node-gyp rebuild. Ensure native build succeeded or install prebuilt binaries.');
}
